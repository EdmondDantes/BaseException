...
Если дочернему классу требуется изменить поведение исключения, он может изменять унаследованные свойства. В данном случае - это является ожидаемым поведением.
...
Конструктор может изменять любые свойства после своего вызова. Поэтому, если вам нужно изменить свойства класса окончательно, делайте это после вызова.

Изменение свойств флажков (с префиксом `is_`) вызывает изменение поведения базового конструктора.
...
Класс `BaseException` не журналируется и не имеет аспектов. Поэтому для наследования используются дополнительные "базовые исключения":
...
(подробнее о журнализировании в разделе: "Журнализирование")
...
Исключения могут попадать в журнал, и хранится в памяти намного дольше, чем объекты, переданные в исключение. Таким образом, вы будете раздувать ваше приложение, так как сборщик мусора не сможет освободить память. Если для Web-скриптов это не актуально, то для фоновых заданий - весьма.
...
Более подробно об отладочном режиме смотрите в разделе: "Отладочный режим".
...
Для вычисления источника для `PHP` `\Exception` используйте метод `BaseException::get_source_for()`.
